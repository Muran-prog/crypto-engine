        ...
        try:
            ...
            with SecureBytes(block_key) as secure_block_key:
                ...
                ciphertext = await loop.run_in_executor(
                    None, lambda: encryptor.update(plaintext) + encryptor.finalize()
                )
...
async def decrypt_block(
    encrypted_block: EncryptedBlock, master_key: bytes, config: CryptoConfig
) -> Dict[str, Any]:
    """
    ...
    with secure_memory_context():
        ...
        try:
            ...
            block_key = await derive_block_key(
                master_key, encrypted_block.salt, encrypted_block.id, config
            )
            ...
            with SecureBytes(block_key) as secure_block_key:
                ...
                plaintext = await loop.run_in_executor(
                    None,
                    lambda: (
                        decryptor.update(encrypted_block.encrypted_data)
                        + decryptor.finalize()
                    ),
                )
