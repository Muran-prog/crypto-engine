...
class AsyncCryptoEngine:
    ...
    async def authenticate_user(
        ...
        """
        success, master_key = await authenticate_user(password, account_info, self.config)
    ...
    async def decrypt_block(self, encrypted_block: EncryptedBlock) -> Dict[str, Any]:
        ...
        return await decrypt_block(encrypted_block, bytes(self._master_key), self.config)
    ...
    async def change_master_password(
        ...
        try:
            ...
            if self._account_info:
                self._account_info.last_password_change = datetime.now(timezone.utc).isoformat()
    ...
    async def delete_account(self, password: str, account_info: AccountInfo) -> bool:
        ...
        try:
            ...
            logger.error(f"An unexpected error occurred during account deletion: {str(e)}")
    ...
    async def bulk_delete_entries(self, deleted_count: int) -> None:
        ...
        if self._account_info and deleted_count > 0:
            self._account_info.entry_count = max(0, self._account_info.entry_count - deleted_count)
            logger.info(f"Account entry count updated. Removed {deleted_count} entries.")
